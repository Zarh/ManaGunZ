	...
	while (eject_running)
	{
        ...
        if (emu_mode == EMU_PSX_MULTI) {
            ...
            if(ret == 0) {
                ...
                if(ret == 0) {eject_running = 1; ret = sys_storage_ext_mount_discfile_proxy(result_port, command_queue, emu_mode & 15, discsize, 256*1024, num_tracks, tracks);}
...
static void rawseciso_thread(uint64_t arg)
{
	...
	ret = sys_storage_ext_mount_discfile_proxy(result_port, command_queue, emu_mode & 15, discsize, 256*1024, num_tracks, tracks);
	DPRINTF("mount = %x\n", ret);
