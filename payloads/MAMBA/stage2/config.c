...
#define COBRA_CONFIG_FILE	"/dev_hdd0/vm/mamba_cfg.bin"
...
static void check_and_correct(MambaConfig *cfg)
{
	...
	if (!found) cfg->bd_video_region = 0;
	else found = 0;
	...
	if (cfg->size > sizeof(MambaConfig)) cfg->size = sizeof(MambaConfig);
}
...
int read_mamba_config(void)
{
	...
	if (config.size > 4096 || checksum(&config) != config.checksum) memset(&config, 0, sizeof(config));		
	else check_and_correct(&config);
...
int write_mamba_config(void)
{
	int fd;
	uint64_t w;
	int ret = 0;
	
	if (cellFsOpen(COBRA_CONFIG_FILE, CELL_FS_O_WRONLY|CELL_FS_O_CREAT|CELL_FS_O_TRUNC, &fd, 0666, NULL, 0) != 0) return ENODEV;
	
	if (cellFsWrite(fd, &config, sizeof(config), &w) != 0) ret = ENODEV;
...
int sys_read_mamba_config(MambaConfig *cfg)
{
	...
	if (cfg->size > 4096) return EINVAL;	
	
	erase_size = cfg->size-sizeof(config.size);
	if (erase_size < 0) erase_size = 0;
