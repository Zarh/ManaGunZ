...
/
/--------------------------------------------------------------------------/
/ Code blocks enclosed with #ifdef _DISK_IMAGE ... #endif is hooking of
...
#ifdef _DISK_IMAGE
FIL ImgFile;	/* Disk image file. It needs to be opened with prior to accsess it. */
#endif
...
DSTATUS disk_initialize (
	BYTE drv		/* Physical drive number (0:SD, 1:Image file in SD) */
)
{
	BYTE n, cmd, ty, ocr[4];
...
#ifdef _DISK_IMAGE
	if (drv == _DISK_IMAGE) {
		if (f_lseek(&ImgFile, f_tell(&ImgFile)) != FR_OK) return STA_NOINIT;	/* Is the image file opened? */
		return (ImgFile.flag & FA_WRITE) ? 0 : STA_PROTECT;
	}
#endif
...
#ifdef _DISK_IMAGE
	if (drv == _DISK_IMAGE && count > 0) {
		if ((sector + count) * 512 > f_size(&ImgFile)) return RES_ERROR;	/* Is the range valid? */
		if (f_lseek(&ImgFile, sector * 512) != FR_OK) return RES_ERROR;		/* Sector offset in the image file */
		if (f_read(&ImgFile, buff, count * 512, &count) != FR_OK) return RES_ERROR;	/* Get sectors from the image file */
		return RES_OK;
	}
#endif
	if (drv != 0 || count == 0) return RES_PARERR;		/* Check parameter */
...
#ifdef _DISK_IMAGE
	if (drv == _DISK_IMAGE && count > 0) {
		if (!(ImgFile.flag & FA_WRITE)) return RES_WRPRT;	/* Read-only image? */
		if ((sector + count) * 512 > f_size(&ImgFile)) return RES_ERROR;	/* Is the range valid? */
		if (f_lseek(&ImgFile, sector * 512) != FR_OK) return RES_ERROR;		/* Sector offset in the image file */
		if (f_write(&ImgFile, buff, count * 512, &count) != FR_OK) return RES_ERROR;	/* Put sectors into the file image */
		return RES_OK;
	}
#endif
	if (drv != 0 || count == 0) return RES_PARERR;		/* Check parameter */
...
)
{
	...
	if (drv == _DISK_IMAGE) {
		...
		case GET_SECTOR_COUNT:
			if (f_lseek(&ImgFile, f_tell(&ImgFile)) == FR_OK) {
				*(LBA_t*)buff = f_size(&ImgFile) / 512;
				res = RES_OK;
			}
			break;
		case GET_BLOCK_SIZE:
			*(DWORD*)buff = 1;
			res = RES_OK;
			break;
		case CTRL_OPEN_IMAGE_RO:
			res = f_open(&ImgFile, buff, FA_READ) == FR_OK ? RES_OK : RES_ERROR;
			break;
		case CTRL_OPEN_IMAGE_RW:
			res = f_open(&ImgFile, buff, FA_READ | FA_WRITE) == FR_OK ? RES_OK : RES_ERROR;
			break;
		case CTRL_CLOSE_IMAGE:
			res = f_close(&ImgFile) == FR_OK ? RES_OK : RES_ERROR;
			break;
		default:
			res = RES_PARERR;
		}
...
#endif
	if (drv != 0) return RES_PARERR;			/* Check parameter */
	if (Stat & STA_NOINIT) return RES_NOTRDY;	/* Check if drive is ready */
