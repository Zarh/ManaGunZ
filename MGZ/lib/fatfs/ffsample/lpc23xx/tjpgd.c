...
#define uint8_tCLIP(v) Clip8[(uint16_t)(v) & 0x3FF]
...
)
{
	...
	for (i = 0; i < 8; i++) {
		...
		dst[0] = uint8_tCLIP((v0 + v7) >> 8);	/* Descale the transformed values 8 bits and output */
		dst[7] = uint8_tCLIP((v0 - v7) >> 8);
		dst[1] = uint8_tCLIP((v1 + v6) >> 8);
		dst[6] = uint8_tCLIP((v1 - v6) >> 8);
		dst[2] = uint8_tCLIP((v2 + v5) >> 8);
		dst[5] = uint8_tCLIP((v2 - v5) >> 8);
		dst[3] = uint8_tCLIP((v3 + v4) >> 8);
		dst[4] = uint8_tCLIP((v3 - v4) >> 8);
		dst += 8;
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		for (iy = 0; iy < my; iy++) {
			...
			for (ix = 0; ix < mx; ix++) {
				...
				*rgb24++ = /* R */ uint8_tCLIP(yy + ((int16_t)(1.402 * CVACC) * cr) / CVACC);
				*rgb24++ = /* G */ uint8_tCLIP(yy - ((int16_t)(0.344 * CVACC) * cb + (int16_t)(0.714 * CVACC) * cr) / CVACC);
				*rgb24++ = /* B */ uint8_tCLIP(yy + ((int16_t)(1.772 * CVACC) * cb) / CVACC);
			}
		}
...
#define	LDB_uint16_t(ptr)		(uint16_t)(((uint16_t)*((uint8_t*)(ptr))<<8)|(uint16_t)*(uint8_t*)((ptr)+1))
...
)
{
	...
	if (LDB_uint16_t(seg) != 0xFFD8) return JDR_FMT1;	/* Err: SOI is not detected */
	ofs = 2;

	for (;;) {
		/* Get a JPEG marker */
		if (jd->infunc(jd, seg, 4) != 4) return JDR_INP;
		marker = LDB_uint16_t(seg);		/* Marker */
		len = LDB_uint16_t(seg + 2);	/* Length field */
		...
		case 0xC0:	/* SOF0 (baseline JPEG) */
			...
			jd->width = LDB_uint16_t(seg+3);		/* Image width in unit of pixel */
			jd->height = LDB_uint16_t(seg+1);		/* Image height in unit of pixel */
