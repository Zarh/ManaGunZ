...
void list(char *path, int depth)
{
    ...
    if (pdir) {
        ...
        while ((ddir = readdir(pdir)) != NULL) {
            ...
            sprintf(new_path, "%s/%s", path, ddir->d_name);
            stat(new_path, &st);
            // List the entry
            ...
            if (ddir->d_type == DT_DIR) {
                ...
                //list(new_path, depth + 1); // recursive list
                     ...
                     if (ddir->d_type == DT_REG) {
                DPrintf(" F %s%s (%lu)\n", indent, ddir->d_name, (unsigned long int)st.st_size);
            } else {
                DPrintf(" ? %s%s\n", indent, ddir->d_name);
            }
