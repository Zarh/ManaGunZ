...
if(sys_storage_open(BD_DEVICE, &g_fd) != 0) {
 ...
 if(get_dec_key(d1, d2)==RET_TRUE)
 {
  ...
  {
    fprintf(g_file_log, "Failed to open disc.key for writing\r\n");
  ...
  if (fwrite(d2, 1, 16, g_file_binlog) != 16) {
    fprintf(g_file_log, "Failed writing binary key 2 to disk.key\r\n");
  }

  if (fwrite(pic, 1, 0x73, g_file_binlog) != 0x73) {
    fprintf(g_file_log, "Failed writing binary pic data to disk.key\r\n");
  }
