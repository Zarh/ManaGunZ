    ...
    UINT32 aMatchMaxLen, UINT32 aKeepAddBufferAfter, UINT32 aSizeReserv)
{
  FreeMemory();
    ...
    if ((unsigned int) m_BlockSize + 256 > kMaxValForNormalize)
      return E_INVALIDARG;
...
UINT32 CInTree::GetLongestMatch(UINT32 *aDistances)
{
  ...
  if ((unsigned int) (m_Pos + m_MatchMaxLen) <= m_StreamPos)
  ...
  UINT32 aMatchMinPos = ((unsigned int) m_Pos > m_HistorySize) ? (m_Pos - m_HistorySize) : 1;
...
void CInTree::NormalizeLinks(CIndex *anArray, UINT32 aNumItems, INT32 aSubValue)
{
  for (UINT32 i = 0; i < aNumItems; i++)
  {
    ...
    if (aValue <= (unsigned int) aSubValue)
