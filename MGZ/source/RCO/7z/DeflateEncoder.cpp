...
HRESULT CCoder::SetEncoderNumPasses(UINT32 A) {
	m_NumPasses = A;
...
HRESULT CCoder::SetEncoderNumFastBytes(UINT32 A) {
	m_NumFastBytes = A;
...
static const int kIfinityPrice = 0xFFFFFFF;
...
int CCoder::WriteTables(bool aWriteMode, bool anFinalBlock)
{
  ...
  if (aWriteMode)
  {
    ...
    while((unsigned int) aNumLitLenLevels > kDeflateNumberOfLitLenCodesMin && aNewLevels[aNumLitLenLevels - 1] == 0)
      aNumLitLenLevels--;
    
    int aNumDistLevels = kDistTableSize;
    while((unsigned int) aNumDistLevels > kDeflateNumberOfDistanceCodesMin && 
    ...
    for (i = 0; i < kLevelTableSize; i++)
    {
      ...
      if (aLevel > 0 && i >= (unsigned int) aNumLevelCodes)
        aNumLevelCodes = i + 1;
    ...
    else
    {
      ...
      for (i = 0; i < (unsigned int) aNumLevelCodes; i++)
    ...
    while (true)
    {
      while(true)
      {
        ...
        if (((m_CurrentBlockUncompressedSize >= kBlockUncompressedSizeThreshold || 
              m_ValueIndex >= (unsigned int) kValueBlockSize) && 
