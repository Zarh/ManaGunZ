// Copyright 2007,2008,2010  Segher Boessenkool  <segher@kernel.crashing.org>
// Licensed under the terms of the GNU GPL, version 2
// http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt

#include <string.h>
#include <stdio.h>

#include "types.h"

void bn_print(char *name, unsigned char *a, unsigned int n)
{
	unsigned int i;

	printf("%s = ", name);

	for (i = 0; i < n; i++)
		printf("%02x", a[i]);

	printf("\n");
}

static void bn_zero(unsigned char *d, unsigned int n)
{
	memset(d, 0, n);
}

void bn_copy(unsigned char *d, unsigned char *a, unsigned int n)
{
	memcpy(d, a, n);
}

int bn_compare(unsigned char *a, unsigned char *b, unsigned int n)
{
	unsigned int i;

	for (i = 0; i < n; i++) {
		if (a[i] < b[i])
			return -1;
		if (a[i] > b[i])
			return 1;
	}

	return 0;
}

static unsigned char bn_add_1(unsigned char *d, unsigned char *a, unsigned char *b, unsigned int n)
{
	unsigned int i;
	unsigned int dig;
	unsigned char c;

	c = 0;
	for (i = n - 1; i < n; i--) {
		dig = a[i] + b[i] + c;
		c = dig >> 8;
		d[i] = dig;
	}

	return c;
}

static unsigned char bn_sub_1(unsigned char *d, unsigned char *a, unsigned char *b, unsigned int n)
{
	unsigned int i;
	unsigned int dig;
	unsigned char c;

	c = 1;
	for (i = n - 1; i < n; i--) {
		dig = a[i] + 255 - b[i] + c;
		c = dig >> 8;
		d[i] = dig;
	}

	return 1 - c;
}

void bn_reduce(unsigned char *d, unsigned char *N, unsigned int n)
{
	if (bn_compare(d, N, n) >= 0)
		bn_sub_1(d, d, N, n);
}

void bn_add(unsigned char *d, unsigned char *a, unsigned char *b, unsigned char *N, unsigned int n)
{
	if (bn_add_1(d, a, b, n))
		bn_sub_1(d, d, N, n);

	bn_reduce(d, N, n);
}

void bn_sub(unsigned char *d, unsigned char *a, unsigned char *b, unsigned char *N, unsigned int n)
{
	if (bn_sub_1(d, a, b, n))
		bn_add_1(d, d, N, n);
}

static const unsigned char inv256[0x80] = {
	,
};

static void bn_mon_muladd_dig(unsigned char *d, unsigned char *a, unsigned char b, unsigned char *N, unsigned int n)
{
	unsigned int dig;
	unsigned int i;

	unsigned char z = -(d[n-1] + a[n-1]*b) * inv256[N[n-1]/2];

	dig = d[n-1] + a[n-1]*b + N[n-1]*z;
	dig >>= 8;

	for (i = n - 2; i < n; i--) {
		dig += d[i] + a[i]*b + N[i]*z;
		d[i+1] = dig;
		dig >>= 8;
	}

	d[0] = dig;
	dig >>= 8;

	if (dig)
		bn_sub_1(d, d, N, n);

	bn_reduce(d, N, n);
}

void bn_mon_mul(unsigned char *d, unsigned char *a, unsigned char *b, unsigned char *N, unsigned int n)
{
	unsigned char t[512];
	unsigned int i;

	bn_zero(t, n);

	for (i = n - 1; i < n; i--)
		bn_mon_muladd_dig(t, a, b[i], N, n);

	bn_copy(d, t, n);
}

void bn_to_mon(unsigned char *d, unsigned char *N, unsigned int n)
{
	unsigned int i;

	for (i = 0; i < 8*n; i++)
		bn_add(d, d, d, N, n);
}

void bn_from_mon(unsigned char *d, unsigned char *N, unsigned int n)
{
	unsigned char t[512];

	bn_zero(t, n);
	t[n-1] = 1;
	bn_mon_mul(d, d, t, N, n);
}

static void bn_mon_exp(unsigned char *d, unsigned char *a, unsigned char *N, unsigned int n, unsigned char *e, unsigned int en)
{
	unsigned char t[512];
	unsigned int i;
	unsigned char mask;

	bn_zero(d, n);
	d[n-1] = 1;
	bn_to_mon(d, N, n);

	for (i = 0; i < en; i++)
		for (mask = 0x80; mask != 0; mask >>= 1) {
			bn_mon_mul(t, d, d, N, n);
			if ((e[i] & mask) != 0)
				bn_mon_mul(d, t, a, N, n);
			else
				bn_copy(d, t, n);
		}
}

void bn_mon_inv(unsigned char *d, unsigned char *a, unsigned char *N, unsigned int n)
{
	unsigned char t[512], s[512];

	bn_zero(s, n);
	s[n-1] = 2;
	bn_sub_1(t, N, s, n);
	bn_mon_exp(d, a, N, n, t, n);
}
